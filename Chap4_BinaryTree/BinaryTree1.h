...
BinaryTreeNode<T>* BinaryTree<T>::GetParent(BinaryTreeNode<T>* root,BinaryTreeNode<T>* current)
{
	...
	if((root->leftchild()==current)||(root->rightchild()==current))
		return root;
	//递归寻找父结点
	if((temp=GetParent(root->leftchild(),current))!=NULL) //如果左子树中的最终结果是NULL，即不在左子树
		return temp;
	else return GetParent(root->rightchild(),current);	  //在右子树中找，并返回右子树中检索的结果
}
...
{
	using std::stack;
	StackElement<T> element;						//类StackElement的一个实例化对象
