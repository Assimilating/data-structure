...
{//二叉搜索树的删除
	...
	//被删结点无左子树吗？
	if(NULL==pointer->leftchild())  //左子树空
	{
		//被删除结点是根结点吗？
		if(NULL==parent)
			root=pointer->rightchild();			//类的数据成员的维护root
	...
	else temppointer=pointer->leftchild();		//左子树非空
	//在左子树中找对称序的最后一个结点
	while(temppointer->rightchild()!=NULL)		//用左子树中的最大节点代替被删除节点
		temppointer=temppointer->rightchild();	//找到左子树中最大的节点
	//被删除结点的右子树作为temppointer的右子树
	temppointer->right=pointer->rightchild();	//被删除节点的右子树成为其左子树中最大节点的右子树
	//被删除结点的左子树根代替被删除结点
	if(parent==NULL)							//课本P118的未优化的方法，被删除节点的左子树的树根直接代替被删除节点
